...
import ProgramModal from '@/components/programs/ProgramModal.vue';
...
const deleteProgram = (item: Program) => {
    ...
    Swal.fire({
        ...
        html: `El programa <strong>${item.name}</strong> será eliminado permanentemente.`,
        icon: 'warning',
        ...
        Swal.fire({
            ...
            router.delete(route('admin.programs.destroy', item.id), {
                preserveScroll: true,
        ...
        <div class="space-y-6 px-6 py-6">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-xl font-bold text-gray-900 dark:text-gray-100">Programas</h1>
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                            {{ programs.total ?? programs.data.length }} programa{{ (programs.total ?? programs.data.length) !== 1 ? 's' : '' }} registrado{{ (programs.total ?? programs.data.length) !== 1 ? 's' : '' }}
                        </p>
                    </div>
                </div>
                <button
                ...
                >
                    <BookOpen class="h-4 w-4" /> Nuevo Programa
                </button>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
                    <thead>
                        <tr class="bg-linear-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-800">
                            <th class="px-5 py-3.5 text-left text-xs font-semibold tracking-wide text-gray-500 uppercase dark:text-gray-400">Nivel</th>
                        ...
                        >
                            ...
                            <td class="px-5 py-3.5">
                                <span
                                    ...
                                    :class="item.level === 'undergraduate'
                                        ? 'bg-[#087ab1]/10 text-[#087ab1]'
                                        : 'bg-violet-100 text-violet-700'"
                                >
                                    {{ item.level === 'undergraduate' ? 'Pregrado' : 'Posgrado' }}
                                </span>
                            </td>
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-2.5">
                                    <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#087ab1]/10">
                                        <BookOpen class="h-4 w-4 text-[#087ab1]" />
                                    </div>
                            ...
                            <td colspan="4" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    <div class="flex h-14 w-14 items-center justify-center rounded-full bg-[#087ab1]/10 dark:bg-[#087ab1]/20">
                                        <BookOpen class="h-7 w-7 text-[#087ab1]/60" />
                                    </div>
                                    <p class="text-sm font-medium text-gray-500">No hay programas registrados</p>
                                </div>
                            </td>
                        </tr>
                    </tbody>
            ...
            <!-- PAGINACIÓN -->
            <div v-if="programs.links && programs.links.length > 3" class="flex justify-end">
