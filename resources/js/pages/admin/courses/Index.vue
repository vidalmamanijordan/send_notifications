...
import { BookMarked, Pencil, Trash2 } from 'lucide-vue-next';
import { ref } from 'vue';
...
const deleteCourse = (item: Course) => {
    ...
    Swal.fire({
        ...
        html: `El curso <strong>${item.name}</strong> será eliminado permanentemente.`,
        icon: 'warning',
        ...
        Swal.fire({
            ...
            router.delete(route('admin.courses.destroy', item.id), {
                preserveScroll: true,
...
<template>
    <AppLayout>
        ...
        <div class="space-y-6 px-6 py-6">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-center gap-3">
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-[#087ab1] shadow-md">
                        <BookMarked class="h-5 w-5 text-white" />
                    </div>
                    <div>
                        <h1 class="text-xl font-bold text-gray-900 dark:text-gray-100">Cursos</h1>
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                            {{ courses.total ?? courses.data.length }} curso{{ (courses.total ?? courses.data.length) !== 1 ? 's' : '' }} registrado{{ (courses.total ?? courses.data.length) !== 1 ? 's' : '' }}
                        </p>
                    </div>
                </div>
                <button
                ...
                >
                    <BookMarked class="h-4 w-4" /> Nuevo Curso
                </button>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
                    <thead>
                        <tr class="bg-linear-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-800">
                            ...
                            <th class="px-5 py-3.5 text-left text-xs font-semibold tracking-wide text-gray-500 uppercase dark:text-gray-400">Créditos</th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700/60">
                        ...
                        >
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-2.5">
                                    <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#087ab1]/10">
                                        <BookMarked class="h-4 w-4 text-[#087ab1]" />
                                    </div>
                            ...
                            <!-- Créditos -->
                            <td class="px-5 py-3.5">
                                <span class="inline-flex items-center rounded-full bg-[#087ab1]/10 px-2.5 py-0.5 text-xs font-medium text-[#087ab1]">
                                    {{ item.credits }} cr.
                                </span>
                            </td>
                            ...
                            <td colspan="5" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    <div class="flex h-14 w-14 items-center justify-center rounded-full bg-[#087ab1]/10 dark:bg-[#087ab1]/20">
                                        <BookMarked class="h-7 w-7 text-[#087ab1]/60" />
                                    </div>
                                    <p class="text-sm font-medium text-gray-500">No hay cursos registrados</p>
                                </div>
                            </td>
                        </tr>
                    </tbody>
            ...
            <!-- PAGINACIÓN -->
            <div v-if="courses.links && courses.links.length > 3" class="flex justify-end">
